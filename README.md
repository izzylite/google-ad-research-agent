...
- **Service-line strict.** An `Exclusions:` field deterministically drops off-service keywords (e.g., chiropractor / PT / pain management) before they reach the report — not just LLM-prose guidance. Morphological forms auto-expand (e.g., `dental` also catches `dentist`, `dentistry`).
- **Acronym-collision aware.** Built-in dictionary catches the invisible budget burn from ambiguous acronyms — `PIP` in a US auto-insurance brief auto-suggests blocking `personal independence payment`, `disability benefit`, `universal credit` (Google's wider indexing); `EMC` blocks `electromagnetic compatibility`, `emc testing`. Operator-editable dictionary at `references/acronym-collisions.json`.
- **Smart brand-conquest deferral.** At constrained budgets (< $200/day default), competitor brand-navigational keywords are deferred from the launch list and surfaced as Consider-negative candidates instead — conquesting competitor brands costs 2-3x normal CPC and needs Smart Bidding data you don't have on Day 1. Override with `Brand conquest: yes` in the brief once budget supports it.
